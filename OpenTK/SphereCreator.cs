...
namespace OpenTK
{
    ...
    public class MeshGeometry3D
    {
        ...
        public Dictionary<int, List<Vector3>> TempNormals = new Dictionary<int, List<Vector3>>();
        ...
        public void Save(string filename)
        {
            using (StreamWriter sw = new StreamWriter(filename))
            {
                ...
                for (int f = 0; f < TriangleIndices.Count; f++)
                {
                    TriangleIndices[f]++;
                }
                for (int f = 0; f < TriangleIndices.Count / 3; f++)
                {
                    sw.WriteLine("f " + TriangleIndices[f * 3] + " " + TriangleIndices[f * 3 + 1] + " " + TriangleIndices[f * 3 + 2]);
                }
                sw.Close();
            }
        }
